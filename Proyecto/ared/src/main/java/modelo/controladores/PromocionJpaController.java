...
public class PromocionJpaController implements Serializable {
        ...
        try {
            ...
            for (PagoAlumno pagoAlumnoCollectionOldPagoAlumno : pagoAlumnoCollectionOld) {
                if (!pagoAlumnoCollectionNew.contains(pagoAlumnoCollectionOldPagoAlumno)) {
                    ...
                    illegalOrphanMessages.add("You must retain PagoAlumno " + pagoAlumnoCollectionOldPagoAlumno + " since its promocion field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
    ...
    public void destroy(PromocionPK id) throws IllegalOrphanException, NonexistentEntityException {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ...
            List<String> illegalOrphanMessages = null;
            Collection<PagoAlumno> pagoAlumnoCollectionOrphanCheck = promocion.getPagoAlumnoCollection();
            for (PagoAlumno pagoAlumnoCollectionOrphanCheckPagoAlumno : pagoAlumnoCollectionOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Promocion (" + promocion + ") cannot be destroyed since the PagoAlumno " + pagoAlumnoCollectionOrphanCheckPagoAlumno + " in its pagoAlumnoCollection field has a non-nullable promocion field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
